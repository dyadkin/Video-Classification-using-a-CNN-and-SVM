...
checkpointer = ModelCheckpoint(
    filepath=os.path.join('data', 'checkpoints', 'SF_MultiRes.{epoch:03d}-{accuracy:.2f}.hdf5'),
    monitor='accuracy',
    verbose=1,
    save_best_only=True)
...
# Helper: Save results.
timestamp = time.time()
csv_logger = CSVLogger(os.path.join('data', 'logs', 'SF_MultiRes' + '-' + 'training-' + \
    str(timestamp) + '.log'))
...
def get_model(weights='imagenet'):
    ...
    #cn = AveragePooling2D((2,2),padding='same')(cn)
    cn = Flatten()(cn)
...
def main(weights_file):
    ...
    validation_generator = None #get_validation_generator()
